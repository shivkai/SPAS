...
const Myinfo = ({role,student}) => {
  ...
  console.log("Inside info "+data[0]);
  return (
  ...
  <div className="flex-grow text-gray-800">
    <main className="p-6 sm:p-10 space-y-6">
      <div className="flex flex-col space-y-6 md:space-y-0 md:flex-row justify-between">
        <div className="mr-6">
          <h1 className="text-3xl text-center font-semibold mb-2">{role.substring(0,1).toUpperCase()+role.substring(1)} Dashboard</h1>
        </div>
      ...
      <section className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-4">
        <div className="flex items-center justify-start md:p-10 py-8 px-5   bg-white shadow rounded-lg">
          ...
          <div>
            <span className="block text-2xl font-bold">{(student && student.value)?student.value.length:0}</span>
          ...
          <div className="overflow-y-auto" >
            <ul className="p-6 space-y-6">
              {/* {data?(.map((std)=>{
                ...
                <span className="text-gray-600 text-xl font-semibold">{std.name}</span>
